 ...
 *
 * ПОЧЕМУ 400 px ПО ДЛИННОЙ СТОРОНЕ И КАЧЕСТВО 0.72. Обложек больше шестидесяти,
 * и они решают не только вес страницы, но и то, останется ли она на телефоне
 * жива. Вес растёт со стороной линейно, а вот память под распакованные
 * картинки - квадратично: 460 px это около мегабайта на обложку в памяти, и
 * на весь набор набегает под шестьдесят. Телефон такого не держит и
 * перезагружает вкладку. 400 px срезают четверть этой памяти, а слот обложки
 * занимает около 200 CSS-px, то есть на экран двойной плотности хватает.
 * В вёрстке к этому добавлены ленивая загрузка и content-visibility на полках,
 * поэтому за экраном обложки не качаются и не распаковываются вовсе.
 */
...
for (const [file, name] of Object.entries(MAP)) {
  ...
  const out = await page.evaluate(async ({
    ...
    return {
      base64: canvas.toDataURL("image/webp", 0.72).split(',')[1],
