 ...
 *
 * Было 0.3, стало 0.9: лента из почти одних мелких сумм выглядела скучной
 * полосой цифр. Ноль тут ставить нельзя - лента из сплошных джекпотов читается
 * как реклама, а не как чужая игра, и доверие к ней падает быстрее, чем растёт
 * интерес. Десятая часть обычных выпадений эту разницу и держит.
 */
export const FEED_BIG_SHARE = 0.9;
...
/**
 * Какую часть ленты отдаём настоящим выпадениям.
 *
 * Раньше настоящие и выдуманные просто складывались и сортировались по
 * свежести. Пока игроков мало, это работает, но стоит кому-то открыть подряд
 * четыре десятка кейсов - и лента целиком состоит из его же выпадений: они
 * самые свежие, и выдуманные вытесняются подчистую. Витрина превращается в
 * личную историю, причём чаще всего из мелочи.
 *
 * Поэтому доля ограничена. Настоящие выпадения всегда есть в ленте и всегда
 * стоят выше при равной свежести, но занять её целиком не могут.
 */
export const FEED_REAL_SHARE = 0.5;
