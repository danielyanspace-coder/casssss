...
/**
 * ЛЕСТНИЦА НОМИНАЛОВ: филлер, полоса окупаемости и хвост. Всего 20 ступеней.
 *
 * ЧТО ЗДЕСЬ ВАЖНО. Отдача задана точно (0.70) и переложить её больше, чем
 * есть, нельзя: средний прокрут возвращает 70 копеек с рубля при любой
 * раскладке. Но одну и ту же отдачу можно разложить очень по-разному, и от
 * раскладки зависит не «сколько игрок проиграет», а «за сколько прокрутов».
 *
 * ПРЕЖНЯЯ РАСКЛАДКА была хвостовой: половина прокрутов возвращала меньше
 * четверти цены, зато крупные множители заходили часто (больше 1.6x - каждый
 * двадцатый раз). Деньги уходили рывками: короткая серия пустых прокрутов
 * съедала депозит целиком раньше, чем игрок успевал что-то почувствовать.
 *
 * СЕЙЧАС середина собрана в одну плотную полосу ЧУТЬ НИЖЕ ЦЕНЫ КЕЙСА. Около
 * двух прокрутов из трёх возвращают 75-98% ставки, то есть почти окупаются;
 * каждый десятый выходит в плюс; остальное - филлер, дешёвая ступень.
 * Игрок теряет те же 30 копеек с рубля, но теряет их ровно, а не обвалом.
 *
 * ЧЕМ ЗАПЛАЧЕНО, И ЭТО НЕ МЕЛОЧЬ. Деньги, которые пошли в полосу
 * окупаемости, взяты из хвоста: выигрыш больше 1.6x стал примерно вдесятеро
 * реже. Потолок кейса не изменился и его шанс сохранён (см. beta профилей),
 * но «выиграл вдвое» перестал быть частым событием. Сумма всех исходов
 * постоянна, иначе не бывает.
 *
 * ВАЖНО ДЛЯ ПРАВОК: среднее число прокрутов до нуля зависит ТОЛЬКО от отдачи
 * (банкролл / (1 - rtp)) и формой лестницы не меняется. Форма меняет разброс:
 * коротких обвальных сессий становится меньше, длинных везучих - тоже.
 * Хотите действительно больше прокрутов на депозит - это правка rtp в
 * описании кейсов, а не лестницы.
 */

/** Дешёвая ступень. Её вероятность решается уравнением, а не задаётся. */
const LADDER_FILLER = 0.12;

/**
 * Полоса окупаемости: восемь ступеней вокруг цены кейса.
 *
 * Веса - форма ГОРБА внутри полосы, а не вероятности: они нормируются на
 * долю полосы из профиля. Горб смещён влево от единицы намеренно - «почти
 * вернул» должно случаться заметно чаще, чем «вышел в плюс», иначе кейс
 * кормит игрока, а не наоборот.
 */
const PAYBACK_STEPS   = [0.75, 0.82, 0.88, 0.93, 0.98, 1.06, 1.18, 1.40];
const PAYBACK_WEIGHTS = [1.15, 1.45, 1.45, 1.20, 0.90, 0.60, 0.35, 0.18];

/**
 * С какого множителя начинается хвост и сколько в нём ступеней.
 *
 * Хвост не задан числами: потолки у кейсов разные, от 60 до 500, и одна
 * таблица на всех либо вылезала бы за потолок дешёвого кейса, либо не
 * дотягивала до потолка дорогого. Поэтому он растягивается геометрически от
 * TAIL_FROM до потолка - у каждого кейса свой, а форма одна.
 */
const TAIL_FROM = 1.55;
const TAIL_STEPS = 10;

/** Полная лестница кейса: филлер, полоса, хвост. Потолок добавляется 20-м. */
function ladderFor(maxMultiplier) {
  const ratio = Math.pow(maxMultiplier / TAIL_FROM, 1 / (TAIL_STEPS + 1));
  const tail = [];
  for (let i = 1; i <= TAIL_STEPS; i++) tail.push(TAIL_FROM * Math.pow(ratio, i));
  return [LADDER_FILLER, ...PAYBACK_STEPS, ...tail];
}

/** Сколько ступеней лестницы приходится на полосу окупаемости. */
const PAYBACK_COUNT = PAYBACK_STEPS.length;

/*
 * ВЕСА ХВОСТА - ломаный степенной закон w ~ m^(-alpha) до перелома и
 * m^(-beta) после.
 *
 * Одним показателем оба конца не описать. Крутой нужен внизу хвоста: без
 * него 2x выпадал бы почти так же часто, как окупаемость, и полоса теряла
 * бы смысл. Но тот же крутой показатель на потолке кейса даёт вес меньше
 * миллионной доли, и обещанный максимум перестаёт выпадать вовсе - карточка
 * обещает 360x, а он не случается никогда.
 *
 * Поэтому beta пологий, и он тут выполняет ещё одну работу. Хвост теперь
 * беден по вероятности, и при прежнем beta = 1.1 шанс потолка упал бы вдвое
 * против старой раскладки. Пологий beta возвращает его на место: доля
 * хвостовой отдачи, приходящаяся на верхние ступени, растёт. Джекпот
 * остаётся примерно тем же событием, что и был, - раз на 20-30 тысяч
 * открытий, - меняется только середина хвоста.
 */
const WEIGHT_BREAK = 5;

/**
 * Профили разброса.
 *
 * evShare - ДОЛЯ МАТОЖИДАНИЯ ПРЕДМЕТОВ, уходящая в полосу окупаемости.
 * Именно доля матожидания, а не вероятности, и это принципиально: полоса
 * задаётся тем же способом, что и плюшки (третий принцип в шапке файла), и
 * поэтому сама подстраивается под кейс. В бонусном кейсе плюшки съедают
 * четверть матожидания - полоса ужимается ровно на эту четверть, и решатель
 * сходится. При доле вероятности, заданной числом, такой кейс просто
 * переставал собираться.
 *
 * Вероятность полосы получается делением: сколько прокрутов около цены
 * укладывается в отведённое матожидание. У soft это почти две трети
 * прокрутов, у wild - меньше половины, зато хвост вдвое гуще.
 */
const PROFILES = {
  soft:   { evShare: 0.895, alpha: 1.70, beta: 0.76 },
  normal: { evShare: 0.870, alpha: 1.55, beta: 0.68 },
  risky:  { evShare: 0.790, alpha: 1.35, beta: 0.60 },
  wild:   { evShare: 0.700, alpha: 1.20, beta: 0.52 },
};

/** Вес ступени хвоста по ломаному закону. Непрерывен в точке перелома. */
function stepWeight(m, profile) {
  if (m <= WEIGHT_BREAK) return Math.pow(m, -profile.alpha);
  return Math.pow(WEIGHT_BREAK, -profile.alpha)
       * Math.pow(m / WEIGHT_BREAK, -profile.beta);
}
...
function buildCase(spec, builtById) {
  ...
  const ladder = ladderFor(maxMultiplier);
  const values = [
  ...
  /*
   * РЕШЕНИЕ ТАБЛИЦЫ. Два неизвестных, два уравнения, ни одной подобранной
   * руками вероятности.
   *
   * Полоса окупаемости забирает фиксированную долю вероятности (profile.mass
   * от той, что осталась после плюшек) и разложена внутри себя по весам-горбу.
   * Хвост задан формой, но не масштабом: множитель s решается. Филлер
   * забирает остаток вероятности.
   *
   *   сумма вероятностей:  p0 + B + s*W = free
   *   матожидание:         p0*v0 + EVb + s*Et = target - perkEv
   *
   * Отсюда s, затем p0. Оба обязаны выйти положительными: если нет -
   * форма полосы не совместима с отдачей, и это ошибка сборки, а не повод
   * подправить число.
   */
  const perkEv = preItems.reduce((s, it) => s + it.probability * it.evValue, 0);
  const free = 1 - perkProbability;
  const v0 = values[0];
  const itemEv = target - perkEv;   // матожидание, оставшееся обычным предметам

  // Полоса окупаемости: ступени с 1-й по PAYBACK_COUNT-ю. Её матожидание
  // задано долей, отсюда вероятность, а не наоборот.
  const bandValues = values.slice(1, 1 + PAYBACK_COUNT);
  const bandWeightSum = PAYBACK_WEIGHTS.reduce((a, b) => a + b, 0);
  const bandAvg = bandValues.reduce((s, v, i) => s + (PAYBACK_WEIGHTS[i] / bandWeightSum) * v, 0);
  const bandEv = profile.evShare * itemEv;
  const bandMass = bandEv / bandAvg;
  const bandProbs = PAYBACK_WEIGHTS.map((w) => (w / bandWeightSum) * bandMass);

  // Хвост: ступени с (1 + PAYBACK_COUNT)-й до потолка включительно.
  const tailMultipliers = [...ladder.slice(1 + PAYBACK_COUNT), maxMultiplier];
  const tailWeights = tailMultipliers.map((m) => stepWeight(m, profile));
  const tailValues = values.slice(1 + PAYBACK_COUNT);
  const tailW = tailWeights.reduce((a, b) => a + b, 0);
  const tailE = tailValues.reduce((s, v, i) => s + tailWeights[i] * v, 0);

  const scale = (itemEv - bandEv - v0 * (free - bandMass)) / (tailE - tailW * v0);
  const p0 = free - bandMass - scale * tailW;

  if (!(scale > 0) || !(p0 > 0) || !(bandMass < free)) {
    throw new Error(
      `[${id}] не удалось свести математику: хвост=${scale.toFixed(6)}, ` +
      `филлер=${p0.toFixed(4)}, полоса=${(bandMass * 100).toFixed(1)}% ` +
      `на среднем ${(bandAvg / price).toFixed(2)}x. Полоса забирает ` +
      `${(profile.evShare * 100).toFixed(0)}% матожидания предметов - либо ` +
      `уменьшить evShare профиля, либо поднять rtp кейса.`
    );
  }

  const tailProbs = tailWeights.map((w) => w * scale);
  ...
  const normalItems = values.map((value, i) => ({
    ...
    probability: i === 0 ? p0
      : i <= PAYBACK_COUNT ? bandProbs[i - 1]
      : tailProbs[i - 1 - PAYBACK_COUNT],
  }));
