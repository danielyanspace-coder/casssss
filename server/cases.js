 ...
 *
 * ЧТО ЗДЕСЬ ВАЖНО. Отдача жёстко равна 0.70, и это ограничение нельзя обойти:
 * сколько бы ступеней ни было, средний прокрут возвращает 70 копеек с рубля.
 * Но одну и ту же отдачу можно разложить очень по-разному, и от раскладки
 * зависит, как игра ощущается.
 *
 * Прежняя лестница начиналась с 0.12 и шла плотно вверх: 0.25, 0.35, 0.5.
 * Две трети прокрутов возвращали меньше половины цены, а «окупился» выпадал
 * в одном случае из семи. Деньги утекали ровной струйкой, без единого повода
 * обрадоваться, - именно на это и жаловались.
 *
 * Теперь между дешёвой ступенью и ценой кейса нет промежуточных: либо прокрут
 * возвращает пятую часть, либо сразу выходит на окупаемость и выше. Мелочь
 * стала одна вместо трёх, зато «окупился и лучше» выпадает примерно каждый
 * пятый раз вместо каждого седьмого, а «половину и больше» - больше чем в
 * половине случаев вместо трети.
 *
 * ЧЕМ ЗАПЛАЧЕНО. При той же отдаче деньги, которые пошли в середину, взяты из
 * хвоста: крупные множители стали реже. Потолок кейса не изменился, изменилась
 * частота подходов к нему. Иначе не бывает - сумма всех исходов постоянна.
 */
const LADDER_LOW = [0.20, 0.60, 0.85, 1.00, 1.20, 1.45, 1.8, 2.25, 2.9, 3.8, 5.0];

/**
 * Сколько ступеней уходит в хвост, между последней постоянной и потолком кейса.
 *
 * Хвост не задан числами: потолки у кейсов разные, от 60 до 500, и одна
 * таблица на всех либо вылезала бы за потолок дешёвого кейса, либо
 * не дотягивала до потолка дорогого. Поэтому хвост растягивается
 * геометрически от 5x до потолка - у каждого кейса свой, а форма одна.
 */
const LADDER_TAIL_STEPS = 8;

/** Полная лестница кейса: постоянная часть плюс хвост под его потолок. */
function ladderFor(maxMultiplier) {
  const from = LADDER_LOW[LADDER_LOW.length - 1];
  const ratio = Math.pow(maxMultiplier / from, 1 / (LADDER_TAIL_STEPS + 1));
  const tail = [];
  for (let i = 1; i <= LADDER_TAIL_STEPS; i++) tail.push(from * Math.pow(ratio, i));
  return [...LADDER_LOW, ...tail];
}
 ...
 */
/*
 * Закон весов ломаный: круто падает вблизи цены кейса и полого дальше.
 *
 * Одним показателем оба конца не описать. Крутой нужен внизу: он собирает
 * вероятность вокруг окупаемости, ради чего лестница и переделана. Но тот же
 * крутой показатель на потолке кейса даёт вес меньше миллионной доли, и
 * обещанный максимум перестаёт выпадать вовсе - карточка обещает 500x, а он
 * не случается никогда. Поэтому после BREAK вес падает уже по пологому beta,
 * и джекпот остаётся редким, но настоящим: примерно раз на тысячу открытий.
 *
 * alpha - крутизна до перелома, beta - после. У осторожных профилей alpha
 * выше: там чаще окупается. У рисковых ниже beta: там богаче хвост.
 */
const WEIGHT_BREAK = 5;

const PROFILES = {
  soft:   { alpha: 2.35, beta: 1.15 },
  normal: { alpha: 2.20, beta: 1.10 },
  risky:  { alpha: 2.00, beta: 1.00 },
  wild:   { alpha: 1.85, beta: 0.90 },
};

/** Вес ступени по ломаному закону. Непрерывен в точке перелома. */
