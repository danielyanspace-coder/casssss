...
const routes = {
  ...
  'GET /api/feed': () => {
     ...
     *
     * Больше половины ленты свои выпадения занять не могут: иначе после
     * четырёх десятков открытий подряд витрина состоит из них одних. То же
     * правило и на сервере - см. FEED_REAL_SHARE.
     */
